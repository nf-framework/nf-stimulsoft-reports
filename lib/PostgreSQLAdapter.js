...
export default async function process(command, onResult, context, provider) {
    try {
        ...
        const tz = context.req.headers['x-user-timezone'] || Intl.DateTimeFormat().resolvedOptions().timeZone;
        resp.data.forEach((row) => {
            row.forEach((col, idx) => {
                if (resp.metaData[idx].dataType == 'date' && resp.metaData[idx].dataSubType == 'timestamptz') {
                    row[idx] = dayjs(col).tz('Asia/Krasnoyarsk').format('YYYY-MM-DDTHH:mm:ss');
                }
            });
        });
